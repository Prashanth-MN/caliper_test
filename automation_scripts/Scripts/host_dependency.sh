#!/bin/bash
host_packages=('libc6' 'libncurses5' 'libstdc++6' 'lib32z1' 'python-dev' 'nfs-common' 'build-essential' 'python-pip' 'automake' 'autoconf' 'make' 'openssh-server' 'libnuma-dev' 'texinfo'  'nfs-kernel-server' 'openjdk-7-jre' 'openjdk-7-jdk')
pip_host_packages=('openpyxl' 'numpy==1.8.2' 'matplotlib==1.3.1' 'Django==1.8.4' )
...
for i in `seq 0 $((${#pip_host_packages[@]}-1)) `
do
	#chcking to see if all the host dependent packages are installed
    check=`pip freeze | grep -c ${pip_host_packages[$i]}`
    if [ $check -eq 0 ] 
    then
	# if force option is passed thn forcefully run the scripts
       if [ $1 = "y" ]
       then
           ...
           echo "${pip_host_packages[$i]} is not installed, would you like to install(y/n)"
           read choice
       fi
       ...
       then
            ...
            sudo pip install ${pip_host_packages[$i]} 
            wait
            if [ $? -ne 0 ]
            then
                echo -e "\n\t\t$ERROR:${pip_host_packages[$i]} is not installed properly"
                exit 1
            fi
       else
           echo -e "\n\t\t$ERROR:Please install ${pip_host_packages[$i]} and try again"
       fi
    else
       echo "${pip_host_packages[$i]} is installed"
    fi
done
